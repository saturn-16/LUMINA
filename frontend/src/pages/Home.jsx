...
import { Search, Film, Music, Calendar, MapPin, Tag, Sparkles } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-blue-950 via-slate-900 to-indigo-950 border border-slate-800 p-8 sm:p-12 mb-10 shadow-2xl">
        <div className="relative z-10 max-w-2xl">
          ...
          <h1 className="text-3xl sm:text-5xl font-black text-white tracking-tight leading-tight mb-4">
            Book the best seats for Movies & Concerts.
          </h1>
          <p className="text-base sm:text-lg text-slate-300 mb-8">
            Interactive visual seat maps, temporary holds with live TTL, zero double-booking, and automated waitlist reallocation on cancellations.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.map((evt) => (
            <Link
              key={evt.id}
              to={`/events/${evt.id}`}
              className="group bg-slate-900 rounded-2xl overflow-hidden border border-slate-800 hover:border-blue-500/50 transition-all duration-200 flex flex-col hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-500/10"
            >
              {/* Event Image */}
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                ...
                <div className="mt-5 pt-4 border-t border-slate-800 flex items-center justify-between">
                  ...
                  <span className="px-3.5 py-1.5 rounded-xl bg-blue-600/10 text-blue-400 border border-blue-500/20 text-xs font-bold group-hover:bg-blue-600 group-hover:text-white transition-all">
                    Select Seats →
                  </span>
                </div>
              </div>
